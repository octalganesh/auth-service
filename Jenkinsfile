                    ...
                    if (env.BRANCH_NAME == 'staging') {
                        ...
                        withCredentials([usernamePassword(credentialsId: '46957a41-b9d8-40ec-8b21-3b41ecca86b9', usernameVariable: 'DEPLOY_USER', passwordVariable: 'DEPLOY_PASS')]) {
                            sh '''
                                ...
                                DEPLOY_DIR=/opt/apps/auth-service
                                REMOTE_JAR=$DEPLOY_DIR/auth-service-0.0.1-SNAPSHOT.jar
                                ...
                                sshpass -p "$DEPLOY_PASS" ssh -o StrictHostKeyChecking=no $DEPLOY_USER@$DEPLOY_HOST "bash -s" << 'EOF'
                                    ...
                                    chmod 775 /opt/apps/auth-service

                                    echo "Stopping old app..."
                                    pkill -f auth-service-0.0.1-SNAPSHOT.jar || true

                                    echo "Starting new app..."
                                    nohup java -jar /opt/apps/auth-service/auth-service-0.0.1-SNAPSHOT.jar \
                                        > /opt/apps/auth-service/auth-service.log 2>&1 &

                                    sleep 5
                                    PID=$(pgrep -f auth-service-0.0.1-SNAPSHOT.jar || true)

                                    if [ -n "$PID" ]; then
                                        echo "✅ Application started with PID $PID"
                                    else
                                        echo "❌ Application failed to start. Check logs at /opt/apps/auth-service/auth-service.log"
                                        exit 1
                                    fi
EOF
                            '''
