                    ...
                    if (env.BRANCH_NAME == 'development') {
                        ...
                        sh '''
                            ...
                            if [ -f "$JAR_FILE" ]; then
                                ...
                                cp "$JAR_FILE" /opt/apps/auth-service-0.0.1-SNAPSHOT.jar
                            else
                            ...
                            echo "Starting app with setsid to detach from Jenkins..."
                             setsid  java -jar /opt/apps/auth-service-0.0.1-SNAPSHOT.jar \
                                > /opt/apps/auth-service/auth-service.log 2>&1 < /dev/null &
                            ...
                            if ps -p $PID > /dev/null; then
                                ...
                                head -n50 /opt/apps/auth-service/auth-service.log
                                exit 1
                            fi
                        '''
                    } else {
